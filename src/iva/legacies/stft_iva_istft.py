...
class IndependentVectorAnalysis:
    ...
    def transform(self, data, W):
        ...
        step = (
            self.fftLen / 2
        )  # /2 # フレーム窓シフト幅(論文[一般で言われているシフト幅]のもう/2しないとダメ?)
        if self.n_components is None:
            self.n_components = data.shape[1]
        elif self.n_components > data.shape[1]:
            self.n_components = data.shape[1]
        ...
        for k in range(K):
            ...
            r[k, :] = np.squeeze(
                np.sum(
                    (
                        np.abs(
                            spectrogram[:, :, :].T.transpose([1, 2, 0])
                            @ W[k, :, :][:, :, None].conj().transpose([1, 0, 2])
                        )
                    )
                    ** 2,
                    axis=0,
            ...
            V = (
                (1 / N_t)
                * (
                    ((fi * spectrogram.T).transpose([1, 0, 2]))
                    @ (spectrogram.conj().transpose([1, 0, 2]))
                )
            ).transpose([1, 2, 0])
            # 分離行列の更新1(solve)
            W[k, :, :] = (
                np.linalg.inv(W.conj().transpose([2, 0, 1]) @ V.transpose([2, 0, 1]))
                @ E[k, :]
            ).T
            # 分離行列の更新2
            W[k, :, :] /= np.sqrt(
                (
                    W[k, :, :][:, :, None].conj().transpose([1, 2, 0])
                    @ V.transpose([2, 0, 1])
                )
                @ W[k, :, :][:, :, None].transpose([1, 0, 2])
            ).squeeze()

        # 分離行列の正規化
        rebuild_spectrogram = (
            W.conj().transpose([2, 0, 1]) @ spectrogram.transpose([1, 2, 0])
        ).transpose([2, 0, 1])
...
if __name__ == "__main__":
    ...
    for i in range(origin_data.shape[1]):
        sf.write("origin_file%d.wav" % i, origin_data[:, i], samplerate, "PCM_16")
